...
────────────────
ЕСЛИ ДАННЫХ МАЛО — НЕ ОТКАЗЫВАЙСЯ
────────────────

Если найдено минимум 3 реальных проекта — всё равно пиши пост.
Автоматически адаптируй формат:
— Топ-5 → Топ-3;
— Топ-10 → Топ-4;
— Топ-7 → Топ-3.

Не объясняй пользователю нехватку данных.
Не пиши оправдания.
Не пиши служебные комментарии.
Просто создай более короткий, но полноценный пост.

────────────────
ЗАПРЕЩЕНО — СЛУЖЕБНЫЕ ФРАЗЫ
────────────────
...
— «данных недостаточно»;
— «я вынужден сообщить»;
— «к сожалению, поиск не дал»;
— объяснения ограничений модели;
— служебные сообщения любого рода;
— комментарии о поиске или генерации;
— обсуждение своих правил.

Пользователь хочет пост — дай пост.
Если данных мало — дай короткий пост из реальных данных.
...
────────────────
ПРОВЕРКА КОНТЕНТА — ЛЮБАЯ СТРАНА
────────────────

Любой проект — российский, американский, корейский, европейский — должен быть подтверждён:
— IMDb;
— Кинопоиском;
— официальным анонсом студии или стриминга;
— крупными СМИ (Variety, Deadline, The Hollywood Reporter и т.д.).

Запрещено использовать:
— малоизвестные проекты без страницы на IMDb;
— generic-названия без источника на любом языке;
— AI/SEO-мусор из поиска, который не имеет официальной страницы.

Приоритет — проектам с подтверждёнными датами релиза и реальными трейлерами.

────────────────
ЗАПРЕЩЁН ТРЕЙЛЕРНЫЙ СТИЛЬ
────────────────

Запрещено писать трейлерные фразы:
— держит в напряжении;
— не отпустит до финала;
— держит за горло;
— невозможно оторваться;
— не даст уснуть;
— пробирает до мурашек.

Пиши конкретные факты о проекте, а не рекламные слоганы.
`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4. Склеиваем перенос после любого эмодзи в пункте списка
    text = text.replace(
      /(\d+\.\s*[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}])/gu,
      (match) => match
    );
    text = text.replace(
      /(\d+\.\s*[🎬🔍🇷🇺🇺🇸🇳🇴🇬🇧🇰🇷🇯🇵🎮🎵🏆🎭🎪🎨🎯🎲🎸🎹🎺🎻🥇🥈🥉⭐💡🔥💰🏅🏋🎤🎧🎼🎦🎞🎟])\s*\n\s*/g,
    ...
    // 5. Склеиваем перенос после жирного названия (**text**\n → **text** )
    text = text.replace(/\*\*([^*]+)\*\*\s*\n\s*/g, "**$1** ");

    // 6. Склеиваем перенос после тире (— \n → — )
    text = text.replace(/—\s*\n\s*/g, "— ");

    // 7. Не более двух переносов подряд
    text = text.replace(/\n{3,}/g, "\n\n").trim();
