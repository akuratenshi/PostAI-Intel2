...
export default async function handler(req, res) {
  ...
  try {
        ...
        messages: [
          {
            role: "user",
            content:
              `Перед написанием постов — найди актуальные новости и события по теме через веб-поиск.\n` +
              `Используй ТОЛЬКО реальные факты из поиска. НЕ выдумывай события, даты, фильмы, продукты.\n\n` +
              prompt,
          },
        ],
      }),
    ...
    // Собираем текст из всех блоков ответа (включая после веб-поиска)
